...
async def save_or_update_user(name: str, email: str, social_security_number: str, application_id: str) -> Dict:
    ...
    Args:
        ...
        email: User's email address (used for login/lookup)
        social_security_number: User's SSN (stored but not used for authentication)
    ...
    try:
        ...
        logger.info(f"[SAVE_USER] Checking for existing user in database by email")
